...
if (n==0)
        ...
        if (myarr.Length % 2 == 0)
    {
        ...
        Console.Write("Было запрошено число: ");
        char[] response = client.DownloadString(string.Format("http://www.randomnumberapi.com/api/v1.0/random?min=0&max={0}&count=1",combined.Length)).ToArray();
        int responsenum = int.Parse(new ArraySegment<char>(response,1,response.Length-3));
        Console.WriteLine(responsenum);
        Sort(combined);
        char[] removednum = removechar(reservecomb,responsenum);
        Console.WriteLine(string.Format("Был удалён элемен под индексом {0}, полученным из API",responsenum));
        Console.WriteLine(string.Format("Строка после удаления {0} элемента выглядит следующим образом:",responsenum));
